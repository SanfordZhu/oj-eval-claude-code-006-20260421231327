...
class ACMOJClient:
    ...
    def submit_code(self, problem_id: int, language: str, code: str, public: bool = False) -> Optional[Dict]:
        data = {"language": language, "code": code, "public": "true" if public else "false"}
        result = self._make_request("POST", f"/problem/{problem_id}/submit", data=data)
        if result and 'id' in result:
            self._save_submission_id(result['id'])
        return result
